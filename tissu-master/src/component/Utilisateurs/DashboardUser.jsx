...
function DashboardUser() {
  ...
  const fetchLogs = useCallback(async (boutiqueId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/logs/boutique/${boutiqueId}`
      );
  ...
  const fetchBoutiqueId = useCallback(
    async (email) => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/boutique/email/${encodedEmail}`
        );
        const data = await response.json();
  ...
  const { tissuId } = useParams();

  // Modifie ton useEffect pour ouvrir le dialogue quand tissuId est présent
  useEffect(() => {
    ...
    if (!jwt) {
      ...
      account
        ...
        .then((response) => {
          ...
          fetchBoutiqueId(response.email).then(() => {
            // Après avoir chargé les tissus, vérifie si un tissuId est dans l'URL
            if (tissuId) {
              const tissuToSell = tissus.find((t) => t.id === tissuId);
              if (tissuToSell) {
                handleOpenSellDialog(tissuToSell);
              }
            }
          });
        })
  ...
  }, [navigate, fetchBoutiqueId, tissuId, tissus]);
  ...
  // Modifie handleOpenSellDialog pour mettre à jour l'URL
  const handleOpenSellDialog = (tissu) => {
    setSelectedTissu(tissu);
    setSellQuantity("");
    setUnitPrice("");
    setOpenSellDialog(true);
    navigate(`/user/${tissu.id}`); // Mettre à jour l'URL
  };

  // Modifie handleCloseSellDialog pour nettoyer l'URL
  const handleCloseSellDialog = () => {
    setOpenSellDialog(false);
    setError(null);
    setSuccessMessage(null);
    navigate("/user"); // Retour à l'URL de base
  };
    ...
    <div>
      ...
      <Container maxWidth="md" sx={{ textAlign: "center", mt: 8 }}>
        ...
        <Typography variant="h6" sx={{ mt: 4 }}>
          Inventaire des Tissus et Logs de vente
        </Typography>
        ...
        <TableContainer component={Paper} sx={{ mt: 2 }}>
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableBody>
              {tissus
                .filter((tissu) => tissu.id.toString().includes(searchValue))
                .map((tissu) => (
                  <TableRow key={tissu.id}>
                    ...
                    <TableCell align="right">
                      <Button
                        variant="contained"
                        color="primary"
                        onClick={() => handleOpenSellDialog(tissu)}
                        component={Link}
                        to={`/user/${tissu.id}`}
                      >
                        Vendre
        ...
        <Dialog open={openSellDialog} onClose={handleCloseSellDialog}>
          ...
          <DialogContent>
            <DialogContentText>
              Veuillez entrer la quantité de{" "}
              <b>{selectedTissu ? selectedTissu.nom : ""} </b> à vendre et le
              prix unitaire en Franc CFA.
            </DialogContentText>
            <TextField
              autoFocus
              margin="dense"
        ...
        <Dialog open={openConfirmDialog} onClose={handleCloseConfirmDialog}>
          ...
          <DialogContent>
            <DialogContentText>
              Vous allez vendre{" "}
              <b>
                {sellQuantity} {selectedTissu ? selectedTissu.unite : ""}
              </b>{" "}
              de <b>{selectedTissu ? selectedTissu.nom : ""}</b> au prix
              unitaire de <b>{unitPrice} Franc CFA</b>.<br />
              Le prix total est{" "}
              <b>
                {selectedTissu
                  ? parseFloat(unitPrice) * parseFloat(sellQuantity)
                  : 0}{" "}
                Franc CFA
              </b>
              .

...
function Boutique() {
    ...
    const fetchBoutiques = useCallback(async (email) => {
        try {
            const encodedEmail = encodeURIComponent(email);
            const response = await fetch(`http://localhost:5000/boutiques/email/${encodedEmail}`);
            const data = await response.json();
            setBoutiques(data);
            if (data.length > 0) {
    ...
    const validateBoutique = () => {
        ...
        if (!newBoutique.telephone) tempErrors.telephone = 'Le téléphone est requis';
        if (!newBoutique.email) tempErrors.email = 'L\'email est requis';
        if (!newBoutique.password) tempErrors.password = 'Le mot de passe est requis';
    ...
    try {
        ...
        if (error.code === 409) {
            console.warn("⚠️ Utilisateur Appwrite déjà existant, on continue avec l'ajout MySQL");
        } else {
            console.error("❌ Échec Appwrite:", error);
            setErrors({ email: "Impossible de créer l'utilisateur Appwrite : " + error.message });
            return;
        }
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            setErrors({ nom: "Erreur serveur lors de l'enregistrement de la boutique." });
        }
    } catch (error) {
    ...
    const handleEditTissuSubmit = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/tissus/${editTissu.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const handleDeleteTissuSubmit = async () => {
        try {
            const response = await fetch(`http://localhost:5000/tissus/${tissuToDelete.id}`, {
                method: 'DELETE'
            });
        ...
        <div>
            ...
            <Container>
                <FormControl fullWidth margin="normal">
                    <InputLabel id="boutique-select-label">Sélectionner une boutique</InputLabel>
                    <Select
                ...
                <Button variant="contained" color="primary" onClick={handleOpenBoutiqueDialog} sx={{ mt: 2 }}>
                ...
                <List>
                    {tissus.length > 0 ? tissus.map((tissu) => (
                        <ListItem key={tissu.id} divider>
                            <ListItemText primary={tissu.nom} secondary={`Stock: ${tissu.stock} ${tissu.unite}`} />
                            <Button variant="contained" color="primary" onClick={() => handleOpenEditTissuDialog(tissu)} sx={{ ml: 2 }}>
                                Éditer
                            </Button>
                            <Button variant="contained" color="secondary" onClick={() => handleOpenDeleteTissuDialog(tissu)} sx={{ ml: 2 }}>
                                Supprimer
                            </Button>
                        </ListItem>
                    )) : <ListItem><ListItemText primary="Aucun tissu disponible" /></ListItem>}
                </List>
                {selectedBoutique && (
                    <Button variant="contained" color="secondary" onClick={handleOpenTissuDialog} sx={{ mt: 2 }}>
                        Ajouter un tissu
                    </Button>
                )}
            </Container>
            <Dialog open={openBoutiqueDialog} onClose={handleCloseBoutiqueDialog}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Veuillez remplir le formulaire ci-dessous pour ajouter une nouvelle boutique.
                    </DialogContentText>
            ...
            <Dialog open={openTissuDialog} onClose={handleCloseTissuDialog}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Veuillez remplir le formulaire ci-dessous pour ajouter un nouveau tissu.
                    </DialogContentText>
                    ...
                    <Box sx={{ minWidth: 120 }}>
                        <FormControl fullWidth margin="normal">
                            ...
                            {tissuErrors.unite && <Typography color="error">{tissuErrors.unite}</Typography>}
                        </FormControl>
                    </Box>
                </DialogContent>
                <DialogActions>
            ...
            <Dialog open={openDeleteTissuDialog} onClose={handleCloseDeleteTissuDialog}>
